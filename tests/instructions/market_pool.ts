 ...
 * @param poolAddress The address of the Liquidity Pool program-derived
 * address account
 */
...
export async function fundPool(
  ...
  await program.methods
    ...
    .accounts({
      ...
      payerTokenAccount: getAssociatedTokenAddressSync(mint, payer.publicKey),
      payer: payer.publicKey,
      systemProgram: anchor.web3.SystemProgram.programId,
 ...
 * @param payer The Liquidity Provider (local wallet in `Anchor.toml`)
 * @param pool The address of the Liquidity Pool program-derived address account
 * @param receiveMint The address of the mint the user is requesting to receive
 * in exchange
 * @param payMint The address of the mint the user is offering to pay in the
 * swap
 * @param quantity The quantity of the mint the user is offering to pay
 * @param decimals The decimals of the mint the user is offering to pay
 * (used to calculate real quantity)
 */
